...
def initialize(context):
    ...
    context.stock = sid(6295) # add some specific securities    
    context.max = 10000
    context.min = 0
    context.profit = 0.02
...
def handle_data(context, data):
    ...
    (data[context.stock].price > (1+context.profit)*context.portfolio.positions[context.stock].cost_basis) :
...
##    if (data[context.stock].price < data[context.stock].vwap(3) or \
##    data[context.stock].price > data[context.stock].vwap(20)) and \
    if data[context.stock].price < data[context.stock].vwap(30) and \
    (today > context.last_sold_date + 10) :
